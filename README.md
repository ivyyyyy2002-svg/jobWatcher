...
> 真正做到一发布就提醒；LinkedIn 只给日期不给时间，没法分钟级，所以退而求其次
> "当天新增推一次"。这是 LinkedIn 数据本身的限制，不是脚本能绕过的。
...
改 `ALERT_WINDOW_MINUTES`（默认 `45`）。这个值要比你的跑动间隔稍大一点，
留出余量，免得 GitHub 定时器偶尔延迟导致漏掉。比如每 30 分钟跑、窗口设 45，
就有 15 分钟容错。去重数据库会兜底防重复，所以窗口重叠也不会重复推送。
（注意：此窗口只对有精确时间的来源生效；LinkedIn 只给日期，走"当天新增推一次"。）
